...
<body>
	...
	<div class="container" id="div-id" style="margin-top:100px;">
        <form action="attendance.php" id="form" method="POST">
				...
				<div class="form-group">
						<label id="label-id">Select student:</label>
						<select class="custom-select my-1 mr-sm-2 select_class" id="inlineFormCustomSelectPref" name="student">
							<?php

								$connection=mysqli_connect("localhost","root","","student_attendance");
								$query="SELECT student.st_id ,student.name from student,student_class,class where student.st_id = student_class.st_id and student_class.cls_id = class.cls_id";
								$qryobj=mysqli_query($connection,$query);
								while($row=mysqli_fetch_assoc($qryobj)){
				...
				<div class="form-group">
                    ...
                    <select class="custom-select my-1 mr-sm-2 select_class" id="inlineFormCustomSelectPref" name="subject">
						<?php
							$connection=mysqli_connect("localhost","root","","student_attendance");
							$query="SELECT subjects.sub_id ,subjects.title from subjects,subject_class,class where subjects.sub_id = subject_class.sub_id and subject_class.cls_id = class.cls_id";
							$qryobj=mysqli_query($connection,$query);
							while($row=mysqli_fetch_assoc($qryobj)){
					...
					<div class="form-group">
                    	...
                    	<input type="date" class="form-control" placeholder="" id="phone" name="date">
					</div>
